...
namespace SMTPE
{
    public partial class IncomingFG : MaterialForm
    {
        ...
        {            
            ...
            //get latest batch
            string queryBatch = "SELECT batchFg FROM tbl_pcblist WHERE woPtsn ='" + cmbWO.Text + "' ORDER BY batchFg DESC";
            using (MySqlDataAdapter adpt1 = new MySqlDataAdapter(queryBatch, connectionDB.connection))
            {
                ...
                {
                    paletNumber = Convert.ToInt32(dset1.Rows[0]["batchFg"].ToString()) + 1;
                }
                else
                {
                    paletNumber = 1;
        ...
        private void ExportLabelScanToExcel()
        {
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Cell(1, 1).Value = "AOHAI FG INCOMING";
                    ...
                    worksheet.Cell(9, 1).Value = "QTY PCB        : " + tbMax.Text;
                    ...
                    worksheet.Range(worksheet.Cell(cellRowIndex, 1), worksheet.Cell(dataGridViewIncomingFG.Rows.Count + cellRowIndex, 9)).Style.Font.FontName = "Times New Roman";
                    worksheet.Range(worksheet.Cell(cellRowIndex, 1), worksheet.Cell(dataGridViewIncomingFG.Rows.Count + cellRowIndex, 9)).Style.Font.FontSize = 9;

                    // storing Each row and column value to excel sheet  
                    for (int i = 0; i < dataGridViewIncomingFG.Rows.Count; i++)
                    {
                        for (int j = 0; j < dataGridViewIncomingFG.Columns.Count; j++)
                        {
                            if (j == 0)
                            {
                            ...
                            {
                                worksheet.Cell(i + cellRowIndex, j + cellColumnIndex).Value = dataGridViewIncomingFG.Rows[i].Cells[j].Value.ToString();
                            }
                        }
                    }

                    int endPart = dataGridViewIncomingFG.Rows.Count + cellRowIndex;
                    ...
                    worksheet.Range(worksheet.Cell(cellRowIndex, 2), worksheet.Cell(endPart - 1, 6)).Style.Border.LeftBorder = XLBorderStyleValues.Thin;
                    ...
                    workbook.SaveAs(directoryFile + "\\IncomingFG\\" + woPTSN.Text + "-" + paletNumber + ".xlsx");
                }

                MessageBox.Show(this, "Excel File Success Generated", "Generate Excel", MessageBoxButtons.OK, MessageBoxIcon.Information);
                System.Diagnostics.Process.Start(@"" + directoryFile + "\\IncomingFG\\" + woPTSN.Text + "-" + paletNumber + ".xlsx");
            }
            catch (Exception ex)
            {
                // tampilkan pesan error
        ...
        private void IncomingFG_Load(object sender, EventArgs e)
        {
        ...
        private void IncomingFG_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void tbPCBSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (tbPCBSN.Text != "")
                {
                    ...
                    {
                        MessageBox.Show("Wrong PCBSN, please fill PCBSN Properly", "WARNING", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        tbPCBSN.Text = string.Empty;
                    }
                }
            }
                                ...
                                if (dset.Rows.Count > 0)
                                {
                                    ...
                                    using (MySqlDataAdapter dscmd1 = new MySqlDataAdapter(query, connectionDB.connection))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            //update Status
                                            string updateisnstatus = "UPDATE tbl_labellist SET STATUS = 'fg', fgDate = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', fgBy = '" + idUser + "', paletName = '" + tbPalet.Text + "' WHERE yuansuanban = '" + tbYuansuanban.Text + "'";
                                            cmd.CommandText = updateisnstatus;
                                            cmd.ExecuteNonQuery();

                                            //insert table
                                            string insertpcblist = "INSERT INTO tbl_pcblist values (NULL,'" + tbWO.Text + "','" + tbPCBSN.Text + "','" + tbMarking.Text + "','" + tbYuansuanban.Text + "','" + cmbWO.Text + "',null, null, '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + idUser + "',null,'" + paletNumber + "','" + session.Text + "','fg' )";
                                            cmd.CommandText = insertpcblist;
                                            int a = cmd.ExecuteNonQuery();

                                            if (a > 0)
                                            {
                                                MessageBox.Show("Saved", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                string queryLabelList = "SELECT id, woCust, pcbsn, marking, yuansuanban, fgdate FROM tbl_pcblist WHERE SESSION = '" + session.Text + "'";
                                                ...
                                                using (MySqlDataAdapter adpts = new MySqlDataAdapter(queryLabelList, connectionDB.connection))
                                                {
                                                    ...
                                                    dataGridViewIncomingFG.DataSource = dsets.Tables[0];
                                                }
                                                qtyData.Text = dataGridViewIncomingFG.Rows.Count.ToString();
                                                ...
                                                if (tbMax.Text == qtyData.Text)
                                                {
                                                    ...
                                                    tbPalet.Text = cmbWO.Text + "/" + paletNumber.ToString();
                                                    qtyData.Text = "0";
                                                    dataGridViewIncomingFG.DataSource = null;
                                                    dataGridViewIncomingFG.Refresh();
                                                }
                                            }
                                            connectionDB.connection.Close();
                                            tbPCBSN.Focus();
